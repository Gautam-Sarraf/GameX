...
export function TournamentCard({ tournament, onRegisterClick, isRegistered }: TournamentCardProps) {
  ...
  const statusBadge = {
    upcoming: 'border-yellow-500/30 text-yellow-400 bg-yellow-950/20 shadow-[0_0_10px_rgba(234,179,8,0.1)]',
    live: 'border-red-500/30 text-red-400 bg-red-950/20 shadow-[0_0_10px_rgba(239,68,68,0.2)] animate-pulse',
    completed: 'border-emerald-500/30 text-emerald-400 bg-emerald-950/20 shadow-[0_0_10px_rgba(16,185,129,0.1)]',
    draft: 'border-zinc-700 text-zinc-400 bg-zinc-800/40'
  };

  return (
    <Card className="group overflow-hidden transition-all duration-300 hover:translate-y-[-4px] hover:border-zinc-700/80 hover:shadow-orange-950/10 hover:shadow-[0_4px_25px_rgba(249,115,22,0.05)] border-zinc-900 bg-zinc-950/50">
      
      {/* Banner */}
      <div className="relative h-40 w-full overflow-hidden bg-zinc-900 border-b border-zinc-900">
        ...
        {/* Game Title tag */}
        <span className="absolute bottom-3 right-3 px-2.5 py-1 text-[10px] font-bold bg-black/60 border border-zinc-800 text-orange-400 rounded-md backdrop-blur-md uppercase tracking-wider">
          {tournament.game}
        </span>
      </div>
      ...
      <CardContent className="p-5">
        ...
        <div className="flex items-center space-x-2 text-xs text-zinc-400 mb-2">
          ...
          <span className="font-semibold text-zinc-300 hover:text-white transition-colors">{tournament.organizerName}</span>
        </div>

        {/* Title */}
        <h4 className="text-base font-extrabold text-white mb-3 line-clamp-1 group-hover:text-orange-400 transition-colors">
          {tournament.title}
        </h4>

        {/* Stats Row */}
        <div className="grid grid-cols-2 gap-3 mb-4 p-3 rounded-lg bg-zinc-900/40 border border-zinc-900/60">
          <div>
            <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider flex items-center">
              <Trophy className="h-3 w-3 mr-1 text-yellow-500" /> Prize Pool
            </span>
            ...
            <div className="h-1.5 w-full bg-zinc-900 rounded-full overflow-hidden border border-zinc-800/40">
              <div
                ...
                className={cn(
                  ...
                  isSlotsFull ? 'bg-zinc-600' : 'bg-gradient-to-r from-orange-500 to-amber-400 shadow-[0_0_8px_rgba(249,115,22,0.4)]'
                )}
              />
            </div>
          </div>
